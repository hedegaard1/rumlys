...
const TEKSTER = {
  da: {
    ...
    automatik: "Automatik",
    automatik_hint: "Det, der sker af sig selv: hvad bevægelse tænder, tidsplanen, og hvornår lyset slukker. Hver automatik har sine egne lamper, og en lampe hører til én automatik — derfor kan to aldrig komme til at trække i den samme pære. Et nyt rum får én automatik med alle lamperne.",
    ...
    ikon_eget_hint: "Det samme ikon, uanset hvilke lamper kortet viser.",
    ...
    kort_sat_ind: "Kortet er sat ind nederst på {sted} — træk det på plads på fanen",
    kort_flyttet: "Kortet er flyttet til {sted}",
    fjern: "Fjern",
    fjern_kort: "Fjern kortet",
    fjern_kort_spoergsmaal: "Fjern kortet fra {sted}? Rumlys husker dets lamper, hvis det kommer igen.",
    kort_fjernet: "Kortet er fjernet fra {sted}",
    ingen_skrivbare_faner: "Der er ingen faner, Rumlys kan skrive i. Betjeningspaneler i YAML skal rettes i filen.",
    ...
    tidsplan_uden_sensor: "Tidsrummenes lys er det, en sensor tænder med. Der er ingen bevægelsessensor i området, så tidsplanen gør ingenting endnu — kortet, scenerne og «hold lys» virker.",
    tidsplan_sensor_ikke_valgt: "Tidsrummenes lys er det, en sensor tænder med. Automatikken har ingen sensor valgt, så tidsplanen gør ingenting endnu — kortet, scenerne og «hold lys» virker.",
    ...
    ingen_hint_uden_sensor: "Der er ingen bevægelsessensor i området, så kun lys, du selv tænder, slukker af sig selv.",
    ingen_hint_ikke_valgt: "Automatikken har ingen sensor valgt, så kun lys, du selv tænder, slukker af sig selv.",
    ...
    hold_hint_uden_sensor: "Slås til med «Hold lys» øverst på siden eller på kortet. Nedtællingen er så ude af spil, til lyset slukkes i hånden, eller tiden er gået.",
    ...
    ny_automatik: "Ny automatik",
    fjern_automatik: "Fjern automatikken",
    bevaegelse_pille: "bevægelse",
    n_tidsrum: "{n} tidsrum",
    ingen_tidsrum: "ingen tidsrum",
    bevaegelse_taender: "Bevægelse tænder",
    aut_lamper_hint: "Lamperne, denne automatik styrer. En lampe hører til én automatik, så to kan aldrig trække i den samme pære.",
    aut_ingen_lamper: "Rummet har ingen lamper endnu. Vælg dem under «Lamper» ovenfor.",
    aut_sensorer_hint: "Sensorerne, der tænder netop disse lamper. Uden en sensor tænder automatikken ikke af sig selv — men tidsrum med fast lys virker stadig.",
    aut_ingen_sensorer: "Der er ingen sensorer valgt til rummet. Sæt flueben ved dem under «Sensorer» ovenfor.",
    hoerer_til: "Hører til automatikken {navn}",
  ...
  en: {
    ...
    automatik_hint: "What happens by itself: what motion turns on, the schedule, and when the light goes off. Each automation has its own lights, and a light belongs to one automation — so two can never pull at the same bulb. A new room gets one automation with all the lights.",
    ...
    ikon_eget_hint: "The same icon, whichever lights the card shows.",
    ...
    tilfoej: "Add",
    har_kort: "has a card",
    kort_sat_ind: "The card was added at the bottom of {sted} — drag it into place on the tab",
    kort_flyttet: "The card was moved to {sted}",
    fjern: "Remove",
    fjern_kort: "Remove the card",
    fjern_kort_spoergsmaal: "Remove the card from {sted}? Rumlys remembers its lights if it comes back.",
    kort_fjernet: "The card was removed from {sted}",
    ingen_skrivbare_faner: "There are no tabs Rumlys can write to. Dashboards in YAML have to be edited in the file.",
    ...
    tidsplan_uden_sensor: "A period's light is what a sensor turns on with. There is no motion sensor in the area, so the schedule does nothing yet — the card, the scenes and «keep the light on» still work.",
    tidsplan_sensor_ikke_valgt: "A period's light is what a sensor turns on with. The automation has no sensor chosen, so the schedule does nothing yet — the card, the scenes and «keep the light on» still work.",
    ...
    ingen_hint_uden_sensor: "There is no motion sensor in the area, so only light you turn on yourself switches off by itself.",
    ingen_hint_ikke_valgt: "The automation has no sensor chosen, so only light you turn on yourself switches off by itself.",
    ...
    hold_hint_uden_sensor: "Turned on with «Keep on» at the top of the page or on the card. The countdown is then out of play until the light is turned off by hand or the time is up.",
    ...
    genindlaes: "Reload",
    ny_automatik: "New automation",
    fjern_automatik: "Remove the automation",
    bevaegelse_pille: "motion",
    n_tidsrum: "{n} time ranges",
    ingen_tidsrum: "no time ranges",
    bevaegelse_taender: "Motion turns on",
    aut_lamper_hint: "The lights this automation controls. A light belongs to one automation, so two can never pull at the same bulb.",
    aut_ingen_lamper: "The room has no lights yet. Choose them under «Lights» above.",
    aut_sensorer_hint: "The sensors that turn on these lights. Without a sensor the automation does not turn on by itself — but time ranges with fixed light still work.",
    aut_ingen_sensorer: "No sensors are chosen for the room. Tick them under «Sensors» above.",
    hoerer_til: "Belongs to the automation {navn}",
...
// Kortets lamper, scener og ikon. Kortet er en betjeningsflade, ikke en ejer: to kort må gerne
// vise den samme lampe, og der er derfor intet at fordele mellem dem. Indtil 0.5.0 delte kortene
// rummets lamper mellem sig, og et kort, hvis lamper stod på et andet kort, kunne ikke bruges.
export function kortetsValg(kortConfig, rumLamper, valg, rumScener) {
  const c = kortConfig || {};
